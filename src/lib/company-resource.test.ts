  ...
  it("uses canonical company facts and never emits a preview hostname", async () => {
    ...
    expect(body).toMatchObject({
      ...
      company: {
        ...
        telephone: COMPANY_ENTITY_FACTS.publicPhones[0].e164,
        telephones: COMPANY_ENTITY_FACTS.publicPhones.map(({e164}) => e164),
        formattedTelephones: COMPANY_ENTITY_FACTS.publicPhones.map(({display}) => display),
