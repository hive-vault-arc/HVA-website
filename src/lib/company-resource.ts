import { CAPABILITY_SOLUTION_PROGRAM_DETAILS } from "./capabilities-content";
import { getAllPosts } from "./blog";
import { getAllCapabilityProfiles } from "./capabilities";
import { getAllEmployeeProfiles } from "./employee-profiles";
import { getAllNewsArticles, getAllResearchReports } from "./insights";
import { getAllPerspectives } from "./perspectives";
...
const importantPages = {
  ...
  companyProfile: absoluteUrl("/ai/company"),
  companyJson: absoluteUrl("/ai/company.json"),
};

export async function getCompanyResource() {
  const [
    caseStudies,
    posts,
    capabilityProfiles,
  ...
  const organizationFacts = resolveOrganizationFacts(
    approvedOrganization,
    "en",
    {
  ...
  const founderProfiles = employeeProfiles.filter(
    (member) => member.profileType === "coFounder",
  );
  ...
  return {
    ...
    structuredResource: absoluteUrl("/ai/company.json"),
    company: {
...
export type CompanyResource = Awaited<ReturnType<typeof getCompanyResource>>;
